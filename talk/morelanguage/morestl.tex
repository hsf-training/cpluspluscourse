...
\begin{frame}[fragile]
  ...
  \begin{exampleblockGB}{Some examples}{https://godbolt.org/z/nP3jr388z}{\texttt{span}}
    \scriptsize
    \begin{cppcode*}{}
      ...
      std::span<const int, 3> s2a2 = a; // compilation failure, invalid conv.
    \end{cppcode*}
  \end{exampleblockGB}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{exampleblock}{}
    ...
    \begin{cppcode*}{}
      ...
      std::optional<Phone> v = parse_phone(...);
      if (v) {                    // or: v.has_value()
...
\begin{frame}[fragile]
  \frametitlecpp[11]{non-member begin and end for your own types}
  \begin{block}{Any class can benefit from range based loop}
    \begin{itemize}
    \item simply by providing \mintinline{cpp}{begin} and \mintinline{cpp}{end} methods
    \item ideally as hidden friends
    \item returning an iterator type
    \end{itemize}
  \end{block}
  \begin{exampleblockGB}{Practical code}{https://godbolt.org/z/9rrjaEdac}{\texttt{std::begin}}
    \scriptsize
    \begin{cppcode*}{}
  struct MyIntRange {
    std::vector<uint32_t> m_internal;
    ...
    friend auto begin( MyIntRange const& v ) {
      return std::begin( v.m_internal );
    }
    friend auto end( MyIntRange const& v ) {
      return std::end( v.m_internal );
    }
  };
  MyIntRange r{2,10,3};
  for ( auto a : r ) {
...
\begin{frame}[fragile]
  ...
  \begin{block}{Clocks}
    ...
    \item consists of a starting point (or epoch) and a tick rate
      ...
      \end{itemize}
    \item C++ defines several clock types
      \begin{itemize}
...
\begin{frame}[fragile]
  ...
  \begin{exampleblockGB}{How to measure the time spent in some code}{https://godbolt.org/z/7E1ce86sK}{\texttt{timing}}
    \small
    \begin{cppcode*}{}
      #include <chrono>
