...
\begin{frame}[fragile]
  \frametitle{Move semantics : the problem}
  \begin{exampleblock}{Dedicated fficient code}
    \begin{cppcode*}{gobble=6}
      std::vector<int> v, w;
      for (int i = 0; i < 10000; i++) v.push_back(i);
      for (int i = 0; i < 10000; i++) w.push_back(i);
...
\begin{frame}[fragile]
  ...
  \begin{block}{A few important points concerning move semantic}
    \begin{itemize}
    \item if not implemented, it falls back to copy semantic
    ...
    \item move assignement operator is allowed to destroy source
      ...
      \item still, I advice to never leave inconsistent object
      \end{itemize}
    \end{itemize}
...
\begin{frame}[fragile]
  ...
  \begin{block}{Use copy and swap idiom}
    ...
    \item use swap in move assignment
      ...
      \item this forces creation of a local replica of source
      \item move semantic will be used reusing our move constructor
...
\begin{frame}[fragile]
  ...
  \begin{alertblock}{Exercise Time}
    ...
    \item implement move semantic the easy way
    \item check efficiency improvements
    \end{itemize}
  \end{alertblock}
\end{frame}
