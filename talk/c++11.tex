...
\subsection[over]{override and final keywords}

\begin{frame}[fragile]
  \frametitle{Override keyword}
  \begin{block}{Idea}
    \begin{itemize}
    \item make sure you do not accidentally create a new virtual function, when intending to override      
    \end{itemize}
    \begin{cppcode}
      struct Base {
      ...
      struct Derived : Base {
        virtual void some_func(double); // oups !
      };
    \end{cppcode}
  \end{block}
  \begin{exampleblock}{Practically}
    \begin{cppcode}
      struct Derived : Base {
        virtual void some_func(double) override; // error
      };
    \end{cppcode}
  \end{exampleblock}
\end{frame}
...
\begin{frame}[fragile]
  \frametitle{Final keyword}
  \begin{block}{Idea}
    \begin{itemize}
    \item make sure you cannot override a given method
  ...
  \begin{exampleblock}{Practically}
    \begin{cppcode}
      struct Base {
        virtual void some_func(float) final;
      };
      struct Derived : Base {
        virtual void some_func(float) override; // error
      };
    \end{cppcode}
  \end{exampleblock}
\end{frame}
...
\begin{frame}[fragile]
  \frametitle{Constructor inheritance}
  \begin{block}{Idea}
    \begin{itemize}
  ...
  \begin{exampleblock}{Practically}
    \begin{cppcode}
      ...
      DerivedClass *a = new{5};
    \end{cppcode}
  \end{exampleblock}
\end{frame}

\begin{frame}[fragile]
  \frametitle{Member initialization}
  \begin{block}{Idea}
    \begin{itemize}
    \item avoid redefining same default value for members in all constructors
  ...
  \begin{block}{Before lambdas}
    \begin{cppcode*}{}
      ...
      std::transform(begin(v), end(v), begin(v),
                     Incrementer(1));
      for (std::vector<int>::iterator it = begin(v);
           it != end(v);
           it++) std::cout << *it << " ";
      \end{cppcode*}
    \end{block}
