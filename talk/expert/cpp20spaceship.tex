...
\begin{frame}[fragile]
  ...
  \begin{block}{Home-made class}
    \begin{itemize}
      ...
      \item This emphasizes that \mintinline{cpp}{(a<=>b)==0} means that \mintinline{cpp}{a} and \mintinline{cpp}{b} are only logically equivalent
...
\begin{frame}[fragile]
  ...
  \begin{block}{Home-made class}
    \begin{itemize}
      ...
      \item It has NOT added a default implementation for \mintinline{cpp}{==} and \mintinline{cpp}{!=}. Those operators are generally expected to mean \textbf{equal}, rather than \textbf{equivalent}. If \mintinline{cpp}{<=>} does not provide a strong order, it is generally advised not to define \mintinline{cpp}{==}.
    \end{itemize}
  \end{block}
\end{frame}
