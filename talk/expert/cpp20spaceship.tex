...
\begin{frame}[fragile]
  ...
  \begin{block}{Motivation}
    \begin{itemize}
    \item There are frequent situations where one needs \mintinline{cpp}{<} operator for a home-made class. Typically, if you want to make a \mintinline{cpp}{std::vector} of such objects, and sort it. But also if you want to use it as a key for e.g. \mintinline{cpp}{std::set} or \mintinline{cpp}{std::map}.
...
\begin{frame}[fragile]
  ...
  \begin{block}{Different kinds of ordering}
    \begin{itemize}
      \item The type \mintinline{cpp}{std::strong_ordering} is convertible to \mintinline{cpp}{std::partial_ordering}, but not the contrary.
      \item Between the two, we have a class \mintinline{cpp}{std::weak_ordering}, where \mintinline{cpp}{==0} mean that the two compared values are equivalent from a ranking point of view, but not necessarily \textbf{equal}. In a given expression, one cannot substitutes one value for the other and be sure to have the same result.
...
\begin{frame}[fragile]
  ...
  \begin{block}{Home-made class}
    \begin{itemize}
      ...
      \item It has NOT added a default implementation for \mintinline{cpp}{==} and \mintinline{cpp}{!=}. Those operators are generally expected to mean \textbf{equal}, rather than \textbf{equivalent}. If \mintinline{cpp}{<=>} does not enable a strong order, the operator \mintinline{cpp}{==}generally deserves a separate implementation.
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{block}{Default \mintinline{cpp}{<=>} implementation}
    \begin{itemize}
      ...
      \item In the previous example, that would not be relevant, because it will compare the numerators first, and conclude that \mintinline{cpp}{3/6} is greater than \mintinline{cpp}{2/3}:
    \end{itemize}
  \end{block}
  \begin{exampleblock}{Example}
    \begin{cppcode*}{}
...
\begin{frame}[fragile]
    ...
    \begin{block}{Summary}
      \begin{itemize}
        ...
        \item Defaulting \mintinline{cpp}{<=>} additionally allows \mintinline{cpp}{==} and  \mintinline{cpp}{!=}.
        \item The standard library defines a few different kinds of order (strong, weak and partial).
        \item Do not confuse equivalence (\mintinline{cpp}{(a<=>b)==0}) with equality (\mintinline{cpp}{a==b}).      \end{itemize}
    \end{block}
\end{frame}
