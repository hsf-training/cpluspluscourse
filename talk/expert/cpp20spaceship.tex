\subsection[spaceship]{The three-way operator}
...
\begin{frame}[fragile]
  ...
  \begin{block}{Home-made class}
    \begin{itemize}
      \item We convert above the result of \mintinline{cpp}{<=>} to \mintinline{cpp}{std::weak_ordering}, so to emphasize that \mintinline{cpp}{(a<=>b)==0} means that \mintinline{cpp}{a} and \mintinline{cpp}{b} are logically equivalent, but may lead to different results in other expressions (e.g. printing them).
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{block}{Home-made class}
    \begin{itemize}
      \item As one can see from the code above, we have defined only \mintinline{cpp}{<=>}, but \mintinline{cpp}{<}, \mintinline{cpp}{>}, \mintinline{cpp}{<=} and \mintinline{cpp}{=>} work as well. The compiler can express the latter operators in terms of \mintinline{cpp}{<=>}. Of course, one can also provide own implementations.
